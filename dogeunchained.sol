/*
                  ...
                  ▌▒▒█        ▄▀▒▒▒▐        
                 ▐▄▀▒▒▀▀▀▀▄▄▄▀▒▒▒▒▒▐        
               ▄▄▀▒░▒▒▒▒▒▒▒▒▒█▒▒▄█▒▐        oooooooooo.     .oooooo.     .oooooo.    oooooooooooo     
             ▄▀▒▒▒░░░▒▒▒░░░▒▒▒▀██▀▒▌        `888'   `Y8b   d8P'  `Y8b   d8P'  `Y8b   `888'     `8      
            ▐▒▒▒▄▄▒▒▒▒░░░▒▒▒▒▒▒▒▀▄▒▒▌        888      888 888      888 888            888               
            ▌░░▌█▀▒▒▒▒▒▄▀█▄▒▒▒▒▒▒▒█▒▐        888      888 888      888 888            888oooo8          
           ▐░░░▒▒▒▒▒▒▒▒▌██▀▒▒░░░▒▒▒▀▄▌       888      888 888      888 888     ooooo  888    "          
           ▌░▒▄██▄▒▒▒▒▒▒▒▒▒░░░░░░▒▒▒▒▌       888     d88' `88b    d88' `88.    .88'   888       o       
          ▌▒▀▐▄█▄█▌▄░▀▒▒░░░░░░░░░░▒▒▒▐      o888bood8P'    `Y8bood8P'   `Y8bood8P'   o888ooooood8          
          ▐▒▒▐▀▐▀▒░▄▄▒▄▒▒▒▒▒▒░▒░▒░▒▒▒▒▌     
          ▐▒▒▒▀▀▄▄▒▒▒▄▒▒▒▒▒▒▒▒░▒░▒░▒▒▐      ooooo     ooo ooooo      ooo   .oooooo.   ooooo   ooooo       .o.       ooooo ooooo      ooo oooooooooooo oooooooooo.   
           ▌▒▒▒▒▒▒▀▀▀▒▒▒▒▒▒░▒░▒░▒░▒▒▒▌      `888'     `8' `888b.     `8'  d8P'  `Y8b  `888'   `888'      .888.      `888' `888b.     `8' `888'     `8 `888'   `Y8b  
           ▐▒▒▒▒▒▒▒▒▒▒▒▒▒▒░▒░▒░▒▒▄▒▒▐        888       8   8 `88b.    8  888           888     888      .8"888.      888   8 `88b.    8   888          888      888 
            ▀▄▒▒▒▒▒▒▒▒▒▒▒░▒░▒░▒▄▒▒▒▒▌        888       8   8   `88b.  8  888           888ooooo888     .8' `888.     888   8   `88b.  8   888oooo8     888      888 
              ▀▄▒▒▒▒▒▒▒▒▒▒▄▄▄▀▒▒▒▒▄▀         888       8   8     `88b.8  888           888     888    .88ooo8888.    888   8     `88b.8   888    "     888      888 
                ▀▄▄▄▄▄▄▀▀▀▒▒▒▒▒▄▄▀           `88.    .8'   8       `888  `88b    ooo   888     888   .8'     `888.   888   8       `888   888       o  888     d88' 
                   ▒▒▒▒▒▒▒▒▒▒▀▀                `YbodP'    o8o        `8   `Y8bood8P'  o888o   o888o o88o     o8888o o888o o8o        `8  o888ooooood8 o888bood8P'   
                                                                                                                                                        
                                                                                                                                                    By Slim Dusty



    Website: https://www.dogeunchained.finance/
    Telegram: https://t.me/DogeUnchained
    Twitter: https://twitter.com/DogeUnchained    
    The Doge Dex: https://www.dogedex.finance/
    
    Tokenomics: 
    
    Variable Auto Staking Reward, Burn, Liquidity, Charity and Community Wallet tax 
    Liquidity Generation set to the doge / dogeunchained liquidity pool
...
contract DogeUnchained is Context, IERC20, Ownable {
    ...
    string private _name = "Doge Unchained";
    string private _symbol = "DUC";
    uint8 private _decimals = 9;
    
    uint256 public _taxFee = 2; // Rewards
    uint256 private _previousTaxFee = _taxFee;
    uint256 public _sellTaxFee = 4; // Rewards
    ...
    uint256 public _sellCommunityFee = 8;
    ...
    address public uniswapV2Pair; // Doge Unchained / Doge
    address public uniswapV2BNBPair; // Doge Unchained / BNB
    ...
    constructor (address communityAddress, address charityAddress, address liquidityAddress, address swapForwardAddress) public {   // , address swapForwardAddress
    ...
    function setSellFeePercents(uint256 sellTaxFee, uint256 sellLiquidityFee, uint256 sellBurnFee, uint256 sellCommunityFee, uint256 sellCharityFee) external onlyOwner() {
        require((sellTaxFee + sellLiquidityFee + sellBurnFee + sellCommunityFee + sellCharityFee) <= 20, ">Max");
    ...
    function setCommunityAddress(address communityAddress) external onlyOwner() {
    ...
    function setAutoSellForCommunity(bool _enabled) external onlyOwner {
        autoSellForCommunity = _enabled;          
    }

    function setAutoSellForCharity(bool _enabled) external onlyOwner {
    ...
    function _getFeeAmounts(uint256 amount) private view returns (uint256, uint256, uint256, uint256) {
        ...
        uint256 totalFee = _communityFee.add(_charityFee).add(_liquidityFee).add(burnFee);        
        ...
        if(totalFee > 0){
            ...
            charityAmount = amount.mul(_charityFee).div(totalFee);
            burnAmount = amount.mul(burnFee).div(totalFee);
        }
        
        uint256 feeAmount = communityAmount.add(charityAmount).add(burnAmount);
        ...
        return (communityAmount, charityAmount, burnAmount, liquidityAmount);
    }

    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {

        (uint256 communityAmount,uint256 charityAmount, uint256 burnAmount, uint256 liquidityAmount) = _getFeeAmounts(contractTokenBalance);
        
        // Send to community addie
        if(communityAmount > 0){
            if(autoSellForCommunity){
                swapTokensForTokens(communityAmount, _communityAddress, _autoSellAddie);
            }
            else {
        ...
        if(charityAmount > 0){
            if(autoSellForCharity){
                swapTokensForTokens(charityAmount, _charityAddress, _autoSellAddie);
            }
            else {
